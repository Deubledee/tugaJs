...
export class appMenu extends mixinBehaviors([
    ...
    static get _getTrigger() {
        return html`            
            <div class="alt">
                <div id="item-text" valid$=[[!valid]] class="item-text" name="[[itemlabel]]" aria-label="mode-category">
                    [[dataActive]]
                </div>
                <div class="item-btn">
        ...
        <div class="dropdown-content" custom$="[[custom]]" slot="dropdown-content">
            <paper-listbox id="content" custom$="[[custom]]" selected="{{selected}}" value="[[value]]" name="languagesListbox"
                ...
                <dom-repeat repeat items="[[items]]" as="item">
                    <template>
                        <paper-item name="[[item]]" title="[[item]]" class="form-al" on-click="_setValue">
                            [[item]]
                        </paper-item>
                    </template>
                </dom-repeat>
    ...
    clickActive() {
        let { formTable, formData } = this.item,
            dataActive
        if (!!formData) dataActive = formData[formTable.property][0]
        setTimeout(() => {
            let Active = Array.from(this.$.content.children).find(element => element.name === dataActive)
            if (!!Active) Active.click()
            this._setResetValue(dataActive)
        }, 500);
    }
    changeValid(invalid) {
    ...
    _setResetValue(dataActive) {
        this.dataActive = this.value = dataActive
    }
    _setValue(evt) {
        if (this.dataActive !== evt.model.__data.item) {
            this.invalid = false
...
elementsDefine(appMenu.is, appMenu);

Object.keys(config).forEach(function (key) {
    appMenu[key] = config[key];
})
